    ...
    for attempt in range(retries):
        try:
            ...
            if data["status"] == "1":
                ...
                logging.error("1Error getting balance1: %s", data["message"])
                return 0
        except Exception as e:
            if attempt < retries - 1:
                logging.error(
                    f"2Error checking balance, retrying in {delay} seconds: {str(e)}"
                )
                time.sleep(delay)
            else:
                logging.error("3Error checking balance: %s", str(e))
                return 0
